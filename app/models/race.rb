class Race < ApplicationRecord
  ...
  scope :for_racer, ->(racer, for_next_year: false, includes_overgrade: false) {
    ...
    if age == 3 && grade.abbr == 'OP' && racer.net_prize < 500 && month <= 9
    ...
    if includes_overgrade
      ...
      elsif (age == 3 || (age == 4 && (month <= 10 || for_next_year))) && grade.abbr == '5'
        grades = Grade.where(abbr: %w[5 9 16 OP Ⅲ Ⅱ Ⅰ])
      elsif (age == 3 || (age == 4 && month <= 10)) && grade.abbr == '9'
        grades = Grade.where(abbr: %w[9 16 OP Ⅲ Ⅱ Ⅰ])
      elsif grade.abbr == '未'
